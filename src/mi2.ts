                            ...
                            if (record.type == "exec") {
                                ...
                                if (record.asyncClass == "running") {
                                    ...
                                    if (reason == "breakpoint-hit") {
                                        ...
                                        if(parsed.record("frame")){
                                            this.stepInto();
                                        }else if (!this.map.hasLineCobol(parsed.record('frame.fullname'), parseInt(parsed.record('frame.line')))) {
                                            this.continue();
                                        } else {
                                            if (this.verbose) {
                                ...
                                if (record.asyncClass == "thread-created") {
                                    ...
                                    && this.lastStepCommand?.name == "stepInto"){
                                    const hostName = record.output?.find(
                                        (entry: string[]) => entry[0] === "host-name")?.[1];
                                    if (hostName) {
                                        this.map.addFile(hostName);
                                    }
                                }
                            }
                        }
